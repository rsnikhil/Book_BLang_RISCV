...
Even though we use numeric notation like ``3'' and ``16'' both at the
value level and at the type level, these two uses are completely
distinct.  They occur in distinct contexts---value expressions {\vs}
type expressions---and so there is no ambiguity.
...
There is a {\BSV} pseudo-function {\tt valueOf($t_n$)} which, given a
numeric type $t_n$, produces the numeric \emph{value} corresponding to
$n_t$.  This is safe because $t_n$ is statically known to {\bsc} (and
that is why it does not make sense to have an inverse function from
numeric values to numeric types).  It is a pseudo-function, not a true
function, because its ``argument'' is a type, not a value.
...
suitable width) for ``don't care'' values, to stand out during
debugging.
...
Study the code in directory \verb|Ex_04_A/|. Compile and run it.
...
Study the code in directory \verb|Ex_04_B/|. Compile and run it.
...
Note that all four type names begin with an uppercase letter, as
...
Study the code in directory \verb|Ex_04_C/|. Compile and run it.
...
Locate the functions mentioned above in the file
\verb|Code/src_Common/Instr_Bits.bsv| and make sure you understand
them thoroughly.

% ----------------
{\Exercise}

Observe that the file contains similar functions, such as: \\
\begin{tabbing}
\hm \verb|is_legal_OP_IMM()| \hm \= \kill
\hm \verb|is_legal_JAL()|        \> for the JAL opcode  \\
\hm \verb|is_legal_JALR()|       \> for the (JALR opcode \\
\hm \verb|is_legal_OP()|         \> for LUI, AUIPC, ADD, SLT, OR, AND, ... opcodes \\
\hm \verb|is_legal_OP_IMM()|     \> for ADDI, SLTI, ..., ORI, ANDI, ... opcodes \\
\hm \verb|is_legal_LOAD()|       \> for LB, LH, LW, LBU, LHU opcodes \\
\hm \verb|is_legal_Mem()|        \> for SB, SH, SW opcodes
\end{tabbing}

which, in turn, use functions like \verb|instr_rs1()|,
\verb|instr_rs2()|, and \verb|instr_rd()|, which are also in the file.

Study the code and check that the functions are correct, while
referring to the ``RV32I Base Instruction Set'' listing in ``Chapter
24 RV32/64G Instruction Set Listings'' in the RISC-V Unprivileged ISA
specification document~\cite{RISCV_Unpriv_2019_12_13}, which lists 40
RV32I instructions.

A visual check is enough, but feel free to write a testbench and tests
and run them if you feel inspired to do so.
...
Section~\ref{ch_Drum_code}.  For now all we need to know that this is
...
Study the code in directory \verb|Ex_04_D/|.  Compile and run it.
...
Delete the \verb|Bit #(7) opcode_BRANCH| constant definition and the
\verb|is_legal_BRANCH()| function from \verb|Top.bsv|.  Instead,
...
Replace the existing \verb|Makefile| with your own by first copying
the existing one (in \verb|../Include.mk|).  In your \verb|Makefile|,
edit the \verb|BSCPATH| definition to include a path to where the
\verb|Instr_Bits.bsv| file is located. Example:
...
Make sure you can, once again, compile and run the test as in the
previous exercises.

% ----------------
\Exercise

Modify the testbench to test more 32-bit values with other
...
instruction, given a 13-bit immediate field, rs2 and rs1 values, and a
funct3 field:
...
Refer to Figure~\ref{Fig_B_imm} for how a 13-bit PC-offset is encoded
into the 12-bit immediate of a B-type instruction.  Your code should
also check that bit [0] of the PC-offset is zero.

In the testbench, replace the right-hand side of lines like this with
an invocation of your new \verb|mkBRANCH_Instr()|:
...
Instead of a constant like {\tt 3'b000} use the symbolic name
\verb|funct3_BEQ| that is defined in the file \verb|Instr_Bits.bsv|.
