...
The inputs to the Decode stage as depicted in
...
Section~\ref{BSV_enum_types}, the identifier \verb|OpClass| is also a
type constant.\footnote{The identifiers {\tt Bits}, {\tt Eq}, and {\tt
FShow} are all predefined ``typeclass'' constants.  Typeclasses are an
advanced topic which we will not be discussing much in this book.
There is a brief discussion of typeclasses in
Section~\ref{Sec_Typeclasses}.  }
...
functions, where a common function may be used over a family of types.
For example we can write a function to exchange the order of bits in a
bit-vector of any width; its argument and result types would be given
as \verb|Bit#(n)|, where \verb|n| is a type variable.
...
\begin{itemize}
  ...
  \item A pair of forward-slashes (``\verb|//|'') begins a comment
    that spans to the end of the current line.  There are many
    examples of this in the code fragments already shown above.

  \item A region of text spanning multiple lines can be a comment if
    preceded by ``\verb|/*|'' (forward-slash and asterisk) and
    followed by ``\verb|*/|'' (asterisk and forward-slash).  This form
    is often used to ``comment-out'' a region of text during debugging
    or trying out alternatives.
...
structs, vectors, and more.  By \emph{strongly typechecked} we mean
that there are strict compile-time rules about what types of arguments
are allowed for each operator or function, and what type of result
they produce.  These rules disallow misinterpretatio of values,
mistakes like taking the square-root of an ASCII character, or
extracting the struct field from something that does not represent a
struct.  Types are \emph{language-level abstractions}: ultimately both
software and hardware compute with raw bits, but a language's
type-system and type-checking ensure that bits are organized and used
in the intended way.
...
The \emph{bsc} compiler usually picks \verb|'h_AAAA_..._AAAA| (of
suitable width) for ``don't care'' values, for visibility during
debugging.
...
Compile and run the code in directory \verb|Ex_04_A/|.

Please see \verb|README.txt| for more details, including suggestions
...
Compile and run the code in directory \verb|Ex_04_B/|.
...
\section{Useful functions during simulation: {\tt \$display()} and {\tt \$finish()}}

\label{BSV_display}

\index[BSV]{\$display@{\tt \$display} statements}

The \verb|$display()| function in {\BSV} is the same as in Verilog and
SystemVerilog, and similar to \verb|printf()| in C/C++. The first
argument is a string containing formatting directives for the
remaining arguments. The \verb|$finish()| function is used to
terminate simulation with a specified exit value.  Example:
...
This is a complete {\BSV} program with a top-level module \verb|mkTop|
with an {\tt Empty} interface (we will discuss modules and interfaces
in more detail later).  It contains one rule, \verb|rl_once|.  We will
discuss rules in more detail later but, briefly, a rule is a process
that may execute its body repeatedly, forever.  In this case the rule
body contains a \verb|$finish(0)| statement that terminates simulation
after its first execution (and the simulation process exits with value
0 to the operating system).

The {\tt a} and {\tt b} arguments to \verb|$display()| are printed in
binary format, using a 4-character field, filling the field to the
left, if necessary, with the ``0'' character.

The {\tt a==b} argument is an expression whose value is of type {\tt
Bool}.  Here, we are printing its bit-representation with the
``\verb|%d|'' format (decimal integer), so it will print ``0'' for
\verb|False| and ``1'' for \verb|True|.

{\BSV}'s \verb|$display()| goes beyond SystemVerilog's
\verb|$display()| in that arguments can can also be of type \verb|Fmt|
which are formatted objects created by the function \verb|fshow()|
which is predefined on all the standard types including \verb|Bool|.
In our example, we can have it print the \emph{words} \verb|False| and
\verb|True| instead of ``0'' and ``1'' by using \verb|fshow()|; in
this case we omit the ``\verb|%d|'' from the format string:
...
Expressions and statements can be packaged into {\BSV} functions.

The syntax of function declarations is the same as in Verilog,
SystemVerilog and C/C++, specifying precise types for the rsult and
for each argument.  We modify our last example:
...
Note that all four type names begin with an uppercase letter.  This is
true of all types in {\BSV}: type names and enumeration constants begin
with an uppercase letter, other identifiers begin with a lowercase
letter.  (See also Section~\ref{BSV_Syntax_of_Identifiers} on syntax
of identifiers.)
...
Compile and run the code in directory \verb|Ex_04_C/|.
...
\input{Code_Extracts/instr_funct3.tex}
...
Locate the above functions in the file
\verb|Code/src_Common/Instr_Bits.bsv|.  Make sure you understand the
remaining functions in the file, which are all similar.
...
\input{Code_Extracts/OpClass.tex}
...
\input{Code_Extracts/valueOf_XLEN.tex}
