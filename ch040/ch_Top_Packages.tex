...
Here is a minimal BSV program (though it is not very interesting as
...
Do the compile-link-run steps described above, in both Bluesim
simulation and in Verilator Verilog simulation.

For your convenience, the code is in directory \verb|Ex_04_01/Top.bsv|, and a
\verb|Makefile| contains all the commands shown above.

Please consult your instructor and/or Appendix~\ref{apx_resources} for
information on installing \emph{bsc} and \emph{verilator}.
...
For your convenience, the code is in directory \verb|Ex_04_02/|, files
\verb|Top.bsv| and \verb|DUT.bsv| and a \verb|Makefile| contains all the
commands shown above.

% ----------------
\Exercise

Try the compile steps above after removing the ``\verb|-u|'' flag and
see what happens.
...
and binds the name \verb|dut| to its interface.  In the rule, we
invoke \verb|dut|'s methods and, using a ``pattern-matching''
statement, bind the names \verb|title|, \verb|authors|, ... {\etc} to
...
For your convenience, the code is in directory \verb|Ex_04_04/|, files
