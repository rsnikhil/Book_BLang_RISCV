...
If a package \verb|Foo| needs to use a name $x$ defined in some other
...
In SystemVerilog, in an {\tt import} statement, instead of ``{\tt *}''
one can list just the names actually needed by the importing package,
one does not have to make all the names available.  This selective
