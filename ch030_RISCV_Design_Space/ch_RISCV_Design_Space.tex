...
as Fife.}  illustrated in Figure~\ref{Fig_Instr_Exec}:
\begin{figure}[htbp]
...
\index{RISCV!Split-phase memory transaction}
\index{RISCV!Memory access!viewed as a pipeline/queue}
\index{RISCV!Memory access!one or more clock ticks}
\index{RISCV!Memory access!cache hit/miss}
\index{RISCV!Memory access!latency}
...
We also say that all memory transactions are \emph{split-phase}: a
