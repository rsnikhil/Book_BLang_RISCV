...
\begin{center}
 \includegraphics[width=\textwidth]{Fig_Rule_Actions_1}
\end{center}
...
\begin{center}
 \fbox{
  \begin{minipage}{4in}
   ...
   \hmm \hmm {\scriptsize Likely modifies state, affecting enabled
             CAN\_FIRE signals for next iteration}
  \end{minipage}
 }
\end{center}
...
about correctness under all possible choices.

\vspace{1ex}

Nevertheless: the \emph{bsc} compiler removes all non-determinism (in
a transparent and predictable way) when it compiles to hardware.
...
The functional correctness of a {\BSV} program only appeals to
rule-at-a-time semantics, without any reference to clocks.
...
It is only for \emph{temporal properties} (``how fast is the
implementation?'') do we need to appeal to clocks, cycles and
